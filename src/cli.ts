...
const root = (value?: string) => resolve(value ?? process.cwd());
...
configCommand.command("set <key> <value>").option("-d, --dir <path>").action(async (key, value, o) => { const dir = root(o.dir); const config = await loadConfig(dir); if (key === "apiKey") await setSecret(dir, config.provider.apiKeyEnv ?? ({ anthropic: "ANTHROPIC_API_KEY", openai: "OPENAI_API_KEY", deepseek: "DEEPSEEK_API_KEY", huggingface: "HF_TOKEN" } as Record<string, string>)[config.provider.kind] ?? "ARACHNE_API_KEY", value); else { const keys = key.split("."); const target = keys.length === 2 && (keys[0] === "provider" || keys[0] === "agent" || keys[0] === "git") ? (config as any)[keys[0]] : keys.length === 3 && keys[0] === "runtime" && keys[1] === "llamaCpp" ? config.runtime.llamaCpp : undefined; const field = keys.at(-1)!; if (!target || !(field in target)) throw new Error("Use provider.model, agent.maxIterations, git.commitOnSuccess, runtime.llamaCpp.executable, or apiKey"); const current = target[field]; const parsed = typeof current === "boolean" ? value === "true" : typeof current === "number" ? Number(value) : value; if (typeof current === "boolean" && value !== "true" && value !== "false") throw new Error("Boolean values must be true or false"); if (typeof parsed === "number" && !Number.isFinite(parsed)) throw new Error("Expected a number"); target[field] = parsed; await saveConfig(dir, config); } console.log("Configuration saved locally."); });
...
program.command("run <task>").option("-d, --dir <path>").action(async (task, o) => { const dir = root(o.dir); const config = await loadConfig(dir); const server = config.provider.kind === "llamacpp" && config.runtime.llamaCpp.autoStart ? new LlamaCppServer(dir, config) : undefined; try { await server?.start(); const loop = new AutonomousLoop(dir, config, createProvider(config, await resolveApiKey(dir, config))); await loop.run(task); } finally { server?.stop(); } });
