...
function answer(input) {
    if (input === "/help")
        return `${paint.bright("Commands")}\n  /status  Show local runtime state\n  /clear   Redraw the interface\n  /exit    Close Arachne`;
    if (input === "/status")
        return `${paint.ok("● Demo runtime is ready.")} No server, model, files, or commands are used.`;
...
export async function launchUi(options) {
    ...
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
        renderShell(demo);
        console.log(paint.muted("Interactive mode requires a terminal. Use --preview for a static render."));
        return;
    }
    console.clear();
    renderShell(demo);
    ...
    await new Promise(resolve => terminal.on("close", resolve));
}
